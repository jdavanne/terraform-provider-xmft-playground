...
SSHPASS=${xmft_st_account.account1.user.password_credentials.password} sshpass -e sftp -P ${xmft_st_site_ssh.ssh1.port} -oBatchMode=no -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null -b - ${xmft_st_account.account1.user.name}@${xmft_st_site_ssh.ssh1.host} << !
   cd ${xmft_st_subscription_ar.sub2.folder}
   ls
   put $TMPDIR/file.txt ${xmft_st_subscription_ar.sub2.folder}/file.txt-$DATE
   ls
   bye
!
sleep 2
