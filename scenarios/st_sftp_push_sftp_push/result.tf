...
sleep 2
SSHPASS=${xmft_st_account.account2.user.password_credentials.password} sshpass -e sftp -P ${xmft_st_site_ssh.ssh1.port} -oBatchMode=no -o HostKeyAlgorithms=+ssh-rsa -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null -b - ${xmft_st_account.account2.user.name}@${xmft_st_site_ssh.ssh1.host} << !
   ls
